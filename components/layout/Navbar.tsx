...
import { RiCalendarLine, RiMenu3Line, RiCloseLine } from 'react-icons/ri';
    ...
    <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md border-b border-[var(--color-border)] bg-[rgba(5,7,15,0.7)]">
      <div className="container-app flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2">
          <RiCalendarLine className="text-[var(--color-neon-pink)]" size={20} />
        ...
        <nav className="hidden md:flex items-center gap-1">
          {links.map(l => (
            <Link key={l.href} href={l.href} className="px-3 py-1.5 text-sm text-[var(--color-text-muted)] hover:text-[var(--color-neon-pink)] rounded transition-colors">
              {l.label}
            </Link>
          ))}
